...
get() { url="https://fonts.googleapis.com/css2?family=$1:wght@$2"; ttf=$(curl -sS -A "Mozilla/4.0" "$url" | grep -o 'https://fonts.gstatic.com[^)]*' | head -1); curl -sS -o "fonts/$3" "$ttf" && echo "fonts/$3"; }
