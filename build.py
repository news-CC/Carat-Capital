...
#book{{position:relative;background:#0E0C09;
  box-shadow:0 40px 110px -38px rgba(0,0,0,.95),0 4px 18px rgba(0,0,0,.5)}}
...
#pgL::after{{content:"";position:absolute;inset:0;z-index:6;pointer-events:none;
  background:linear-gradient(-90deg,rgba(59,54,44,.22),transparent 7%)}}
...
.pbody{{font-size:12.5px;color:var(--pink2);margin:0;text-align:justify;hyphens:auto;-webkit-hyphens:auto}}
...
.mgrow button{{font:inherit;color:#E8CB7C;background:none;border:1px solid rgba(198,162,74,.35);
  border-radius:999px;padding:7px 13px;cursor:pointer;letter-spacing:.18em;text-transform:uppercase}}
...
/* ── the phone: the same leaves as a swipe deck ── */
@media(max-width:700px){{
  #book{{display:flex;overflow-x:auto;scroll-snap-type:x mandatory;scrollbar-width:none;gap:12px;
    width:min(92vw,440px);background:none;box-shadow:none}}
  #book::-webkit-scrollbar{{display:none}}
  .edge,#cornhint,#under,#peelC,#foldsh{{display:none!important}}
...
<div class="stage"><div id="bookwrap"><div id="zoomer"><div id="book">{leaves}
  ...
  <div id="under"></div>
  <div id="foldsh"></div>
  <div id="peelC"><div id="peel"><div class="inner"></div><div class="curl"></div></div></div>
  <div id="cornhint"></div>
...
<div class="mgnav">
  ...
  <div class="mgrow">
    ...
    <button id="mtoc">Contents</button>
    <button id="mthumbs">Pages</button>
    <button id="mzoom">Zoom</button>
...
(function(){{
  ...
  const NP={npages}, NS=Math.ceil({npages}/2);
  let mode='', cur=0, BW=0, BH=0, anim=null;
  ...
  function sizeBook(){{
    const vw=Math.min(innerWidth-36,1180), vh=innerHeight-186;
    let pw=Math.min(vw/2, vh/1.4);
    BH=pw*1.4; BW=pw*2;
    book.style.width=BW+'px'; book.style.height=BH+'px';
  }}
  function settle(){{
    /* static spread for the current position: verso of the sheet before, recto of this one */
    ...
    pgno.textContent=(cur===0?'cover':(cur===NS?'back':(2*cur)+'\\u2013'+(2*cur+1)))+' / '+NP;
  ...
  function spring(dir,fromP,toP,done){{
    ...
    const dur=matchMedia('(prefers-reduced-motion: reduce)').matches?1:420;
  ...
  function turn(dir){{
    if(mode!=='book'||anim) return;
    if(dir>0&&cur>=NS||dir<0&&cur<=0) return;
    wrap.style.transform='';
    spring(dir,0.02,1,()=>{{ cur+=dir; unfoldStatics(dir); settle(); }});
  }}
  function go(n){{
    if(mode==='book'){{ if(anim){{cancelAnimationFrame(anim.raf);anim=null;unfoldStatics(1);}}
      cur=Math.max(0,Math.min(NS,n)); settle(); }}
    else book.scrollTo({{left:n*book.clientWidth,behavior:'smooth'}});
  }}
  const next=()=>mode==='book'?turn(1):book.scrollBy({{left:book.clientWidth,behavior:'smooth'}});
  const prev=()=>mode==='book'?turn(-1):book.scrollBy({{left:-book.clientWidth,behavior:'smooth'}});

  /* the drag: the fold follows the finger */
  let D=null;
  book.addEventListener('pointerdown',e=>{{
    if(mode!=='book'||anim||document.body.classList.contains('zoomed')) return;
    if(e.target.closest('a')) return;
    const r=book.getBoundingClientRect();
    const x=e.clientX-r.left, fwd=x>r.width/2?1:-1;
  ...
  addEventListener('pointerup',e=>{{
    if(!D) return;
    const {{dir,moved,p,t0,x0}}=D; D=null;
  ...
  document.getElementById('tocp').addEventListener('click',e=>{{
    ...
    mode==='book'?go(+b.dataset.s):go(Math.max(0,2*(+b.dataset.s)-1));}});
  ...
  function setMode(){{
    ...
    if(want==='book'){{
      ...
      scrub.max=slots.length-1; pgno.textContent='1 / '+NP;
    }}
  }}
  const h=location.hash.match(/^#s(\\d+)$/);
  if(h) cur=Math.max(0,Math.min(NS,+h[1]));
  setMode(); buildThumbs();
