...
html,body{{background:#100E0B;overflow-x:clip}}
...
(function(){{
  ...
  pgno.addEventListener('change',()=>{{
    ...
    mode==='book'?go(Math.ceil(Math.max(0,Math.min(NP-1,n-1))/2)):go(Math.max(0,Math.min(NP-1,n-1)));
  }});
  ...
  function setMode(){{
    ...
    if(want==='book'){{
      ...
      scrub.max=slots.length-1; pgno.value='1';
    }}
  }}
  ...
  book.addEventListener('scroll',()=>{{
    ...
    pgno.value=(i+1); scrub.value=i;}},{{passive:true}});
}})();
</script>
</body></html>"""
